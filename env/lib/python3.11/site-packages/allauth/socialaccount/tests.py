...
class SocialAccountTests(TestCase):
    ...
    def test_email_address_created(self):
        ...
        setattr(user, account_settings.USER_MODEL_EMAIL_FIELD, "test@pyseeker.com")
    ...
    def test_email_address_clash_username_required(self):
        ...
        request, resp = self._email_address_clash("test", "test@pyseeker.com")
        self.assertEqual(resp["location"], reverse("socialaccount_signup"))

        # POST different username/email to social signup form
        request.method = "POST"
        request.POST = {"username": "other", "email": "other@pyseeker.com"}
        resp = signup(request)
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@pyseeker.com"}
        )
    ...
    def test_email_address_clash_username_not_required(self):
        ...
        request.method = "POST"
        request.POST = {"email": "other@pyseeker.com"}
        resp = signup(request)
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@pyseeker.com"}
    ...
    def test_email_address_clash_username_auto_signup(self):
        ...
        request, resp = self._email_address_clash("test", "other@pyseeker.com")
        self.assertEqual(resp["location"], "/accounts/profile/")
        user = get_user_model().objects.get(
            **{account_settings.USER_MODEL_EMAIL_FIELD: "other@pyseeker.com"}
        )
    ...
    def _email_address_clash(self, username, email):
        ...
        user_email(exi_user, "test@pyseeker.com")
        exi_user.save()
    ...
    def test_disconnect(self):
        ...
        user_email(user, "test@pyseeker.com")
        user.set_password("test")
        user.save()
    ...
    def test_verified_email_change_at_signup(self):
        ...
        sociallogin = SocialLogin(
            user=User(email="verified@pyseeker.com"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(email="verified@pyseeker.com", verified=True, primary=True)
            ],
        )
        ...
        self.assertEqual(form["email"].value(), "verified@pyseeker.com")
        resp = self.client.post(
            reverse("socialaccount_signup"),
            data={"email": "unverified@pyseeker.org"},
        )
        self.assertRedirects(resp, "/accounts/profile/", fetch_redirect_response=False)
        user = User.objects.all()[0]
        self.assertEqual(user_email(user), "verified@pyseeker.com")
        self.assertTrue(
            EmailAddress.objects.filter(
                user=user,
                email="verified@pyseeker.com",
                verified=True,
                primary=True,
            ).exists()
        )
    ...
    def test_unverified_email_change_at_signup(self):
        ...
        User = get_user_model()
        sociallogin = SocialLogin(
            user=User(email="unverified@pyseeker.com"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(
                    email="unverified@pyseeker.com",
                    verified=False,
                    primary=True,
                )
            ],
        ...
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "unverified@pyseeker.com")
        resp = self.client.post(
            reverse("socialaccount_signup"),
            data={"email": "unverified@pyseeker.org"},
        )
        ...
        user = User.objects.all()[0]
        self.assertEqual(user_email(user), "unverified@pyseeker.org")
        self.assertTrue(
            EmailAddress.objects.filter(
                user=user,
                email="unverified@pyseeker.com",
    ...
    def test_unique_email_validation_signup(self):
        session = self.client.session
        User = get_user_model()
        User.objects.create(email="me@pyseeker.com")
        sociallogin = SocialLogin(
            user=User(email="me@pyseeker.com"),
            account=SocialAccount(provider="google"),
            email_addresses=[
                EmailAddress(email="me@pyseeker.com", verified=True, primary=True)
            ],
        )
        session["socialaccount_sociallogin"] = sociallogin.serialize()
        session.save()
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "me@pyseeker.com")
        resp = self.client.post(
            reverse("socialaccount_signup"), data={"email": "me@pyseeker.com"}
        )
    ...
    def test_social_account_taken_at_signup(self):
        ...
        User = get_user_model()
        sociallogin = SocialLogin(
            user=User(email="me1@pyseeker.com"),
        ...
        resp = self.client.get(reverse("socialaccount_signup"))
        form = resp.context["form"]
        self.assertEqual(form["email"].value(), "me1@pyseeker.com")
        resp = self.client.post(
            reverse("socialaccount_signup"),
            data={"username": "me1", "email": "me1@pyseeker.com"},
        )
