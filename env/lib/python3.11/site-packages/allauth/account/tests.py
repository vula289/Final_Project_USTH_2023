...
class AccountTests(TestCase):
    ...
    def test_username_containing_at(self):
        ...
        EmailAddress.objects.create(
            user=user,
            email="raymond.penners@pyseeker.com",
            primary=True,
            verified=True,
        )
    ...
    def test_signup_same_email_verified_externally(self):
        user = self._test_signup_email_verified_externally(
            "john@pyseeker.com", "john@pyseeker.com"
        )
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 1)
        EmailAddress.objects.get(
            verified=True, email="john@pyseeker.com", user=user, primary=True
        )

    def test_signup_other_email_verified_externally(self):
        """
        John is invited on john@pyseeker.org, but signs up via john@pyseeker.com.
        E-mail verification is by-passed, their home e-mail address is
        used as a secondary.
        """
        user = self._test_signup_email_verified_externally(
            "john@pyseeker.com", "john@pyseeker.org"
        )
        self.assertEqual(EmailAddress.objects.filter(user=user).count(), 2)
        EmailAddress.objects.get(
            verified=False, email="john@pyseeker.com", user=user, primary=False
        )
        EmailAddress.objects.get(
            verified=True, email="john@pyseeker.org", user=user, primary=True
        )
    ...
    def test_signup_email_twice(self):
        ...
        self.assertEqual(user.email, "john@pyseeker.org")
    ...
    def _request_new_password(self):
        user = get_user_model().objects.create(
            username="john", email="john@pyseeker.org", is_active=True
        )
        user.set_password("doe")
        user.save()
        self.client.post(
            reverse("account_reset_password"),
            data={"email": "john@pyseeker.org"},
        )
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].to, ["john@pyseeker.org"])
        return user
    ...
    def test_password_reset_flow_with_another_user_logged_in(self):
        ...
        user2 = self._create_user(username="john2", email="john2@pyseeker.com")
        EmailAddress.objects.create(
    ...
    def test_email_verification_mandatory(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(mail.outbox[0].to, ["john@pyseeker.com"])
        ...
        for attempt in [1, 2]:
            ...
            self.assertEqual(
                EmailConfirmation.objects.filter(
                    email_address__email="john@pyseeker.com"
                ).count(),
                attempt,
            )
            # Wait for cooldown
    ...
    def test_email_escaping(self):
        ...
        if allauth.app_settings.SITES_ENABLED:
            ...
            site.save()
        u = get_user_model().objects.create(username="test", email="user@pyseeker.com")
    ...
    def test_login_unverified_account_optional(self):
        ...
        user.save()
        EmailAddress.objects.create(
            user=user, email="user@pyseeker.com", primary=True, verified=False
        )
        resp = self.client.post(
    ...
    def test_login_failed_attempts_exceeded_cleared_on_password_reset(self):
        ...
        EmailAddress.objects.create(
            user=user, email="john@pyseeker.org", primary=True, verified=True
        )
    ...
    def test_login_using_unverified_email_address_is_prohibited(self):
        ...
        EmailAddress.objects.create(
            user=user, email="john@pyseeker.com", primary=True, verified=False
        )

        resp = self.client.post(
            reverse("account_login"), {"login": "john@pyseeker.com", "password": "doe"}
        )
        self.assertRedirects(
            resp,
        ...
        self.assertEqual(len(mail.outbox), 1)
        assert mail.outbox[0].to == ["john@pyseeker.com"]
    ...
    def test_login_inactive_account(self):
        ...
        user.save()
        EmailAddress.objects.create(
            user=user, email="john@pyseeker.com", primary=True, verified=True
        )
        resp = self.client.post(
            reverse("account_login"), {"login": "john", "password": "doe"}
        )
    ...
    def test_confirm_email_with_another_user_logged_in(self):
        ...
        user = get_user_model().objects.create_user(
            username="john", email="john@pyseeker.org", password="doe"
        )
        self.client.force_login(user)
        self.client.post(
...
class EmailFormTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.user = User.objects.create(username="john", email="john1@pyseeker.org")
    ...
    def test_add(self):
        resp = self.client.post(
            ...
            {"action_add": "", "email": "john3@pyseeker.org"},
        )
        EmailAddress.objects.get(
            email="john3@pyseeker.org",
            user=self.user,
            verified=False,
            primary=False,
        )
    ...
    def test_add_with_two_limiter(self):
        resp = self.client.post(
            reverse("account_email"), {"action_add": "", "email": "john3@pyseeker.org"}
        )
...
class BaseSignupFormTests(TestCase):
    ...
    def test_signup_email_verification(self):
        ...
        data = {
            "username": "username",
            "email": "user@pyseeker.com",
            "email2": "user@pyseeker.com",
        }
        form = BaseSignupForm(data, email_required=True)
        self.assertTrue(form.is_valid())

        data["email2"] = "anotheruser@pyseeker.com"
        form = BaseSignupForm(data, email_required=True)
        self.assertFalse(form.is_valid())
...
class AuthenticationBackendTests(TestCase):
    def setUp(self):
        user = get_user_model().objects.create(
            is_active=True, email="john@pyseeker.com", username="john"
        )
...
class UtilsTests(TestCase):
    ...
    def test_pk_to_url_string_identifies_UUID_as_stringlike(self):
        user = UUIDUser(is_active=True, email="john@pyseeker.com", username="john")
...
class TestCVE2019_19844(TestCase):
    ...
    def test_user_email_unicode_collision(self):
        User = get_user_model()
        User.objects.create_user("mike123", "mike@pyseeker.org", "test123")
        User.objects.create_user("mike456", "mıke@pyseeker.org", "test123")
        data = {"email": "mıke@pyseeker.org"}
        form = ResetPasswordForm(data)
        self.assertTrue(form.is_valid())
        form.save(self.global_request)
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].to, ["mıke@pyseeker.org"])
